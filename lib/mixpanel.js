...
MixpanelAPI = (function() {
        ...
        try {
            ...
            req = http.get(req_opts, __bind(function(res) {
                ...
                res.on('end', __bind(function() {
                    var error, result, json_string;
                    try {
                        result = JSON.parse(this._json_string(endpoint, body));
                        error = null;
                        if (result.error) {
    ...
    MixpanelAPI.prototype._hostname = function(endpoint) {
        if (endpoint === 'export') {
    ...
    MixpanelAPI.prototype._json_string = function(endpoint, body) {
        var json_string = body.join('');
        if (endpoint === 'export') {
